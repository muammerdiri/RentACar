...
namespace Business.Concrete
{
    public class CarImageManager : ICarImageService
    {
        ...
        {
            return new SuccessDataResult<List<CarImage>>(_carImageDal.GetList(), Messages.CarsImagesListed);
        }
        ...
        public IDataResult<List<CarImage>> GetCarImages(int carId)
        {
            ...
            var images = checkIfCarImage.Success
                ...
                : _carImageDal.GetList(c => c.CarId == carId);
        ...
        {
            var deletedImages = _carImageDal.GetList(c => c.CarId == carId);
        ...
        {
            int result = _carImageDal.GetList(c => c.CarId == carId).Count;
            if (result >= 5)
            {
        ...
        private IDataResult<List<CarImage>> CheckIfCarHasImage(int carId)
        {
            ...
            bool result = _carImageDal.GetList(c => c.CarId == carId).Any();
            if (!result)
            {
        ...
        {
            var result = _carImageDal.GetList(c => c.Id == imageId).Any();
            if (!result)
            {
